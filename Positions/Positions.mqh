...
class Position {  
   public:
      ...
      Position() {};
      Position(int ticket, double size, double price, double target=0, double stopLoss=0):
         m_ticket(ticket), m_size(size), m_price(price), m_target(target), m_stopLoss(stopLoss) {};
      
      
      void print() {
         PrintFormat("[Position - %d] %.4f / %.2f  -> %.4f, %.4f", m_ticket, m_price, m_size, m_target, m_stopLoss);
      }
};
...
class Positions {
   ...
   public:
      Positions(string orderType): m_orderType(orderType) { m_positions = new LList<Position>(); };
   ...
   for (int i = 0; i < OrdersTotal(); i++) {
      ...
      if ((this.count() < MAX_POSITIONS) && 
         ...
         if ((m_orderType == "LONG") && (OrderType() == OP_BUY)) {
            this.add(new Position(OrderTicket(), OrderLots(), OrderOpenPrice(), OrderTakeProfit(), OrderStopLoss()));
